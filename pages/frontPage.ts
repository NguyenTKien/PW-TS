...
export class FrontPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.requestMessageSuccess = page.locator(
      "//div[@class='row contact']//h2/following-sibling::p"
    );
  }
  ...
  async verifyRequestMessageSuccess(
    requestSuccessTitle: string,
    requestName: string,
    requestMessageTitle: string
  ) {
    expect(await this.requestSuccessTitle.textContent()).toContain(
      requestSuccessTitle
    );
    expect(await this.requestSuccessTitle.textContent()).toContain(requestName);
    const getMessageSuccessContent = await this.getListMessage(
      this.requestMessageSuccess
    );
