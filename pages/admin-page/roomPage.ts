...
export class RoomPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page)
    this.body = page.locator("//body/div")
    ...
    this.wifiCheckbox = page.getByRole("checkbox").and(page.locator("#wifiCheckbox"))
    ...
    this.checkinEdit = page.locator("//div[contains(@class, 'datepicker')]//input")
    this.checkoutEdit = page.locator("//div[contains(@class, 'datepicker')]//input")
    this.updateBookingInfo = page.locator("//div[contains(@class, 'detail booking')]//div[@class='row']//p")
    this.deleteBookingButton = page.locator("//span[contains(@class, 'bookingDelete')]")
  }
  ...
  async clickToConfirmUpdateBooking() {
    await Promise.all([this.confirmBookingEditButton.click(),
  ...
  async updateBookingRoom(
    ...
    await this.confirmBookingEditButton.waitFor({ state: "visible" })
    ...
    await this.checkoutEdit.last().fill(checkoutDate)
    // await this.page.waitForTimeout(1000)
  }

  async verifyBookingHasUpdated(firstname: string, lastname: string, checkinDate: string, checkoutDate: string) {
    await this.editBookingButton.waitFor({ state: "visible" })
...
export function getRoomDetailsFromAmenities(roomAmenities?: RoomAmenities): string {
  if (!roomAmenities) {
    // Handle undefined or null roomAmenities
